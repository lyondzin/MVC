...
function koneksi() {
    ...
    $koneksi = new mysqli("localhost", "root", "", "pasienn");
...
function getTabelPasien()
{
    ...
    $query = "SELECT * FROM pasien_db";  // SQL query to fetch patient data
...
if (isset($_POST['simpan'])) {
    ...
    $link->query("INSERT INTO pasien_db (idPasien, nmPasien, jk, alamat) VALUES ('$idPasien', '$nmPasien', '$jk', '$alamat') 
...
if (isset($_GET['idPasien'])) {
    ...
    $link->query("DELETE FROM pasien_db WHERE idPasien = '$idPasien'") or die($link->error);
    header("Location: pasien.php");
}
